...
import rospy
import numpy as np
from beginner_tutorials.msg import Joint_position,End_effector_pose
...
rospy.Subscriber("Joint_position", Joint_position, topic_callback)
end_effector_pub=rospy.Publisher("End_effector_pose", End_effector_pose, queue_size=10)
...
def forward_kinematics(joint_array):
    ...
    pose.x,pose.y,pose.z,pose.phi=pE0[0],pE0[1],pE0[2],phi
    end_effector_pub.publish(pose)
    print("Received Joint angles : " ,[joint_array.joint1_position,joint_array.joint2_position,joint_array.joint3_position,joint_array.joint4_position])
